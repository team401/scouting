...
export async function GET(request: Request) {
  ...
  const normalizedMatches = matches.results.map((match) => {
    ...
    const alliances = JSON.parse(String(row.alliances)) as {
  ...
  return Response.json({
    ...
    pitEntries: pitEntries.results.map((entry) => ({
      ...entry,
      motorTypes: entry.motorTypes ? JSON.parse(String(entry.motorTypes)) : [],
      dimensions: entry.dimensions
        ? JSON.parse(String(entry.dimensions))
        : null,
...
export async function POST(request: Request) {
  ...
  if (!eventResponse.ok || !matchesResponse.ok)
    return Response.json(
      {
        error: `TBA could not load that event (${eventResponse.status}/${matchesResponse.status}).`,
      },
      { status: 502 },
    );
  ...
  const statements = [
    ...
    env.DB.prepare(
      'UPDATE events SET is_current = 0, updated_at = ? WHERE organization_id = ?',
    ...
    env.DB.prepare(`INSERT INTO events (id, organization_id, season_year, tba_event_key, name, is_current, tba_etag, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, 1, ?, ?, ?) ON CONFLICT(organization_id, tba_event_key) DO UPDATE SET name = excluded.name, is_current = 1, tba_etag = excluded.tba_etag, updated_at = excluded.updated_at`).bind(
      eventId,
      organizationId,
      event.year,
      event.key,
  ...
  return Response.json({
    event: { key: event.key, name: event.name, year: event.year },
