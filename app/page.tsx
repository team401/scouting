...
export default function Home() {
  ...
  return <main className={dark ? 'dark app-shell' : 'app-shell'}>
    ...
    <section className="min-w-0 flex-1"><header className="topbar"><div><p className="eyebrow">Team 401</p><h1>Qualification 18</h1></div><div className="flex items-center gap-2"><Badge variant="outline" className="status-badge"><CloudOff />Offline ready</Badge>{!isPending && (session ? <Button variant="outline" onClick={() => authClient.signOut().then(() => window.location.reload())}><LogOut />{session.user.name}</Button> : <Button nativeButton={false} render={<a href="/sign-in" />}>Sign in</Button>)}<Button aria-label="Toggle color theme" size="icon" variant="ghost" onClick={() => setDark(!dark)}>{dark ? <Sun /> : <Moon />}</Button></div></header>
