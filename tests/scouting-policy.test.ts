...
import {
  ...
  canOverwriteLockedEntry,
  canReopenEntries,
  scoutEntryMutationId,
  teamNumberFromTbaKey,
} from '../lib/scouting-policy.ts';

void test('enforces assignment and reopening roles', () => {
...
void test('only privileged users or reopened records can overwrite a locked entry', () => {
...
void test('rejects malformed TBA team keys', () => {
...
void test('builds deterministic mutation identifiers', () => {
  const first = scoutEntryMutationId(
    '2026vabla',
    '2026vabla_qm1',
    401,
    'user-1',
  );
  const second = scoutEntryMutationId(
    '2026vabla',
    '2026vabla_qm1',
    401,
    'user-1',
