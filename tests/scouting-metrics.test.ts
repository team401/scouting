...
import {
  ...
  observedPoints,
  summarizeEntries,
} from '../lib/scouting-metrics.ts';

void test('calculates observed REBUILT points', () => {
  assert.equal(
    observedPoints({
      autoFuel: 5,
      activeFuel: 30,
...
void test('calculates median for odd and even samples', () => {
  assert.equal(median([9, 2, 5]), 5);
  assert.equal(median([2, 4, 8, 10]), 6);
  assert.equal(median([]), 0);
});

void test('excludes no-shows from performance aggregates', () => {
